...
def sell():
    ...
    if request.method == "GET":
        user_id = session["user_id"]
        symbols_user = db.execute("SELECT symbol FROM transactions WHERE user_id = :id AND symbol = :symbol GROUP BY symbol HAVING SUM(shares) > 0", id=user_id )
        return render_template("sell.html", symbols = [row["symbol"] for row in symbols_user])
         ...
         user_shares =db.execute("SELECT shares FROM users WHERE id =:id AND symbol = :symbol GROUP BY symbol", user_id, symbol)
