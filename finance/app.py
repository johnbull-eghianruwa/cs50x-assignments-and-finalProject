...
def register():
    ...
    if request.method == "GET":
        ...
        if not password:
            return apology("Must Gice Password")
...
def sell():
    ...
    if request.method == "GET":
        user_id = session["user_id"]
        symbols_user = db.execute("SELECT symbols FROM transaction WHERE user_id = :id GROUP BY SYMBOL HAVING SUM(shares) > 0", id=user_id)
    ...
    user_shares = db.execute("SELECT shares FROM  transactions WHERE user_id=:id AND symbol = :symbol GROUP BY symybol", id=user_id, symbol=symbol)
